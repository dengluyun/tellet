...
{
    //模糊半径
    public static  float BlurRadius = 0;
    //降分辨率
    public static  float downSample = 1;
    //迭代次数
    ...
    void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        ...
        if (_Material)
        {
            ...
            RenderTexture rt1 = RenderTexture.GetTemporary(Convert.ToInt32(source.width / downSample), Convert.ToInt32(source.height / downSample), 0, source.format);
            RenderTexture rt2 = RenderTexture.GetTemporary(Convert.ToInt32(source.width / downSample), Convert.ToInt32(source.height / downSample), 0, source.format);
